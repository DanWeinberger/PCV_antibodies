...
```{r}
d3 <- 
  d2[grepl('child', d2$dose_description) & d2$assay=="GMC" & d2$dose_description=='1m post primary child' & d2$vaccine %in% c('PCV7','PCV13','PCV15'),]
...
test2 <- c2[ c2$dose_description=="1m post primary child" , c('serotype','pct_responders_est','pct_responders_carr', 'vaccine', 'study_id')]  
View(test2)

test2$vaccine <- factor(test2$vaccine, c('PCV7', 'PCV13','PCV15'))

ggplot(test2, aes(x=vaccine, y=pct_responders_carr, group=study_id))+
  geom_point()+
  theme_classic() +
  facet_wrap(~serotype)
